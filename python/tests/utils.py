...
TEST_PROJECT_ROOT = str(
    Path(TEST_FILE_SYSTEM_ROOT).joinpath("etc").joinpath("test-project").absolute()
)
RESOURCE_ROOT = str(Path(TEST_RESOURCE_ROOT).parent.absolute())
SCRIPTS_DIR = str(Path(RESOURCE_ROOT).absolute())
...
def check_file_is_in_deb(deb_file_path: str, file_path: str) -> bool:
    return (
        file_path
        in subprocess.run(
            ["dpkg", "-c", deb_file_path], text=True, stdout=subprocess.PIPE
        ).stdout
    )


def check_files_matches_in_deb(
    deb_file_path: str, files_and_matchers: list[tuple[str, str]]
) -> bool:
